...
const getFutureTargetDate = () => {
  const now = new Date();
  const target = getValidTargetDate();
  while (target.getTime() <= now.getTime()) {
    target.setFullYear(target.getFullYear() + 1);
  }
  if (target.toISOString() !== getStoredTarget()) {
    localStorage.setItem(STORAGE_KEYS.target, target.toISOString());
  }
  return target;
};
...
const syncSettingsInputs = () => {
  const storedTarget = getFutureTargetDate();
...
const getTargetDate = () => getFutureTargetDate();
